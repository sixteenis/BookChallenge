...
class RoomCreateVM: BaseViewModel {
    ...
    func transform(input: Input) -> Output {
        ...
        successImage
            .flatMap {
                LSLPNetworkManager.shared.request(target: .contentPost(content: .init(book: self.bookModel, title: roomTitle.value, deadLine: datePicker.value.formatted(.iso8601), limitPreson: limitPeople.value, content: roomContent.value, files: $0)), dto: ContentPostDTO.self)
            }
